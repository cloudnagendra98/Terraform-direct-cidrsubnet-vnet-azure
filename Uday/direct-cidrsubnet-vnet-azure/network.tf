...
resource "azurerm_subnet" "subnets" {
  ...
  ##address_prefixes     = [var.azure_subnets_range[count.index]] (we put this in comments and write replacement below fr this using cidrsubnet function)
  address_prefixes = [cidrsubnet(var.azure_network_range, 8, count.index)]
  depends_on = [
    azurerm_resource_group.newresg,
